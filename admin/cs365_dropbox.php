  ...
  if (isset($_POST['sc365dp_email'])) {
   ...
   try {
    ...
    function cs365_dropbox_multi() {
     $request = "select * from cs365_external_back WHERE date_inserted is null limit 15";
...
##preparing files and directories...
     ...
     if ($num_mysql > 0) {
      ...
      if ($number_to_go != 0) {
       $tmp_numb_files = $number_to_go - 15;
       if ($tmp_numb_files <= 0)
         $tmp_numb_files = 0;
      }
     }
     ...
     $uploaded = '';
     if ($tmp_numb_files > 0) {
      $perc = 'Percent Complete : ' . round(100 - (($number_to_go / $tmp_numb_files) * 100), 0) . '%';
      $uploaded = intval($tmp_numb_files) - intval($number_to_go);
      if ($uploaded < 0) {
       $uploaded = $uploaded * $uploaded;
      }
      $uploaded = $uploaded . ' of ' . $tmp_numb_files . ' Processed';
     }
